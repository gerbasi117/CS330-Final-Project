...
void ViewManager::ProcessKeyboardEvents()
{
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_1) == GLFW_PRESS)
	{
		...
		g_pCamera->Position = glm::vec3(0.0f, 6.0f, 50.0f);
		g_pCamera->Up = glm::vec3(0.0f, 20.0f, 0.0f);
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_2) == GLFW_PRESS)
	{
		...
		g_pCamera->Position = glm::vec3(50.0f, 6.0f, 0.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_3) == GLFW_PRESS)
	{
		...
		g_pCamera->Position = glm::vec3(0.0f, 16.0f, 0.0f);
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_4) == GLFW_PRESS)
	{
		...
		g_pCamera->Position = glm::vec3(0.0f, 5.5f, 25.0f);
		g_pCamera->Front = glm::vec3(0.0f, -0.5f, -2.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
		g_pCamera->Zoom = 80;
	}
...
void ViewManager::PrepareSceneView()
{
	...
	else
	{
		...
		double scale = 0.0;
		float orthoZoom = 30.0f; // Adjust for zoom level (bigger number = more zoomed out)

		if (WINDOW_WIDTH > WINDOW_HEIGHT)
		{
			scale = (double)WINDOW_HEIGHT / (double)WINDOW_WIDTH;
			projection = glm::ortho(-orthoZoom, orthoZoom, -orthoZoom * (float)scale, orthoZoom * (float)scale, 0.1f, 100.0f);
		}
		else if (WINDOW_WIDTH < WINDOW_HEIGHT)
		{
			scale = (double)WINDOW_WIDTH / (double)WINDOW_HEIGHT;
			projection = glm::ortho(-orthoZoom * (float)scale, orthoZoom * (float)scale, -orthoZoom, orthoZoom, 0.1f, 100.0f);
		}
		else
		{
			projection = glm::ortho(-orthoZoom, orthoZoom, -orthoZoom, orthoZoom, 0.1f, 100.0f);
		}
	}
